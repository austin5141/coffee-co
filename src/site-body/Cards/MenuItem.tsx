...
function MenuItem(props: Props) {
  ...
  return (
    <div key={props.id} className="w-96 md:w-full bg-rose-900 rounded-2xl p-5 mx-auto">
        ...
        {quantity === 0 
          ? (<button onClick={ () => increaseItemQuantity(props.id, props.price, props.itemname) } className="text-white font-poppins ml-10 border-2 mt-2 border-rose-500 rounded-md p-1 transition-all hover:bg-rose-500 hover:scale-125">Add To Cart</button>) 
          : <div className="flex flex-col items-center gap-5 mt-2">
                <div className="flex gap-5">
                  ...
                  <button onClick={ () => increaseItemQuantity(props.id, props.price, props.itemname)} className="bg-rose-500 w-7 h-8 font-poppins font-medium rounded-md text-white">+</button>
                </div>
